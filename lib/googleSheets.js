...
export async function verificarUsuario(email, pin) {
  ...
  try {
    // Forzar lectura directa de la hoja
    console.log('📖 Leyendo hoja Usuarios...');
    const token = await getAccessToken();
    if (!token) {
      console.error('❌ No se pudo obtener token de acceso');
      return null;
    ...
    if (!rows || rows.length <= 1) {
      console.error('❌ No se encontraron datos en la hoja Usuarios');
      return null;
    }
    ...
    // Buscar el email (ignorando espacios y mayúsculas)
    const emailTrimmed = email.trim().toLowerCase();
    console.log('🔍 Buscando email exacto:', emailTrimmed);
    
    const usuarioRow = rows.find((row, index) => {
      if (index === 0) return false; // Saltar encabezados
    ...
    if (!usuarioRow) {
      ...
      console.log('📋 Emails disponibles en la hoja:');
      rows.slice(1).forEach((row, i) => {
        console.log(`  ${i + 1}: ${row[emailIndex] || 'vacío'}`);
      });
      return null;
    }
