...
export async function appendToSheet(sheetName, values) {
  try {
    console.log('📤 appendToSheet - sheetName:', sheetName);
    console.log('📤 appendToSheet - values:', values);
...
export async function getClientes() {
  try {
    ...
    rows.forEach(row => {
      ...
      // Solo crear un expediente si Numero_SAC NO está vacío
      const tieneExpediente = obj.Numero_SAC && obj.Numero_SAC.trim() !== '';

      if (!clientesMap.has(id)) {
        // Crear el cliente con o sin expediente inicial
        const nuevoCliente = {
          ID_Cliente: id,
        ...
        // Si tiene expediente, lo agregamos
        if (tieneExpediente) {
        ...
        // Solo agregar expediente si Numero_SAC NO está vacío
        if (tieneExpediente) {
...
export async function buscarClientes(termino) {
  try {
    ...
    const resultados = clientes.filter(c => {
      // Buscar en campos principales del cliente
      ...
      if (matchPrincipal) {
        console.log('✅ Match principal:', c.Nombre_Cliente);
        return true;
      }
      
      // Buscar en los expedientes del cliente
      if (c.expedientes && c.expedientes.length > 0) {
        const matchExpediente = c.expedientes.some(exp => 
          exp.Numero_SAC?.includes(term) ||
          exp.Caratula?.toLowerCase().includes(term)
        );
        if (matchExpediente) {
          console.log('✅ Match expediente:', c.Nombre_Cliente);
        }
        return matchExpediente;
      }
...
export async function crearCliente(nombre, telefono, dni, domicilio) {
  try {
    console.log('📤 Creando cliente:', { nombre, telefono, dni, domicilio });
...
export async function verificarUsuario(email, pin) {
  console.log('🔍 Verificando usuario:', email, pin);
  ...
  try {
    ...
    const headers = data[0];
    console.log('📋 Headers de Usuarios:', headers);
    ...
    // Buscar el email (ignorando espacios en blanco)
    ...
    console.log('PIN guardado:', pinGuardado, 'PIN ingresado:', pin);
