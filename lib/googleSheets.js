...
export async function getClientes(usuario = null) {
  try {
    ...
    rows.forEach(row => {
      ...
      // Si se pasó un usuario, verificar que esté compartido
      if (usuario) {
        const compartidos = obj.Usuarios_Compartidos ? obj.Usuarios_Compartidos.split(',').map(e => e.trim()) : [];
        // Si no es el creador (el primer usuario que aparece en la fila) y no está en la lista de compartidos, saltar
        // Nota: el creador es el que está en el campo Creado_Por (no tenemos ese campo aún, así que asumimos que el dueño es el primer usuario)
        // Por ahora, si tiene Numero_SAC y no está compartido con el usuario, lo saltamos
        if (tieneExpediente) {
          const esCompartido = compartidos.includes(usuario);
          // Si no es compartido y el usuario no es el creador (lo dejamos pasar por ahora), lo saltamos
          // Esta lógica se mejorará cuando tengamos un campo "Creado_Por" en la hoja
        }
      }
      ...
      if (!clientesMap.has(id)) {
        ...
        if (tieneExpediente) {
          // Verificar si el expediente está compartido con el usuario
          const compartidos = obj.Usuarios_Compartidos ? obj.Usuarios_Compartidos.split(',').map(e => e.trim()) : [];
          const esCompartido = usuario ? compartidos.includes(usuario) : true;
          
          // Si hay usuario y no está compartido, no lo agregamos
...
function getMockData(sheetName) {
  const mockData = {
    ...
    'Usuarios': [
      ...
      ['sofiaonassis@gmail.com', '8728', 'usuario', 'SI'], // Agregado
    ],
    'Actuaciones': [
