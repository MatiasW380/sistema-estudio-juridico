...
async function getRootFolderId() {
  console.log('🔍 Buscando carpeta raíz "SISTEMA DE GESTION"...');
  
  if (ROOT_FOLDER_ID) {
    console.log(`✅ Usando ROOT_FOLDER_ID cacheado: ${ROOT_FOLDER_ID}`);
  ...
  try {
    const token = await getAccessToken();
    if (!token) {
      console.error('❌ No se pudo obtener token para buscar carpeta raíz');
      return null;
    }
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(`❌ Error al buscar carpeta raíz: ${response.status} - ${errorText}`);
      return null;
    }

    const data = await response.json();
    console.log(`📦 Respuesta de Drive:`, JSON.stringify(data, null, 2));

    if (data.files && data.files.length > 0) {
      ROOT_FOLDER_ID = data.files[0].id;
      console.log(`✅ Carpeta raíz encontrada: ${ROOT_FOLDER_ID} (nombre: ${data.files[0].name})`);
      return ROOT_FOLDER_ID;
    } else {
      console.error('❌ No se encontró la carpeta "SISTEMA DE GESTION" en la raíz de Drive');
      console.error('   Asegurate de que la carpeta existe y está compartida con la cuenta de servicio');
      return null;
    }
  } catch (error) {
...
export async function crearCarpetaExpediente(numeroSAC, caratula) {
  ...
  try {
    ...
    console.log('🔍 PASO 1: Obteniendo carpeta raíz...');
    const rootFolderId = await getRootFolderId();
    if (!rootFolderId) {
      console.error('❌ PASO 1 FALLÓ: No se encontró la carpeta raíz SISTEMA DE GESTION');
      return null;
    }
    console.log(`✅ PASO 1 OK: Root folder ID = ${rootFolderId}`);

    // Paso 2: Obtener token de acceso
    console.log('🔑 PASO 2: Obteniendo token de acceso para Drive...');
    const token = await getAccessToken();
    if (!token) {
      console.error('❌ PASO 2 FALLÓ: No se pudo obtener token de acceso');
      return null;
    }
    console.log(`✅ PASO 2 OK: Token obtenido`);

    // Paso 3: Crear la carpeta
    const nombreCarpeta = `${numeroSAC} - ${caratula}`;
    console.log(`📁 PASO 3: Creando carpeta "${nombreCarpeta}" en parent: ${rootFolderId}`);
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(`❌ PASO 3 FALLÓ: Error al crear carpeta: ${response.status} - ${errorText}`);
      return null;
    }

    const data = await response.json();
    console.log(`✅ PASO 3 OK: Carpeta creada: ${nombreCarpeta} (ID: ${data.id})`);
    return data.id;
  } catch (error) {
