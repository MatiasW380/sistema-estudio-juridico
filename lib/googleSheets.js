...
export async function getClientes(usuario = null) {
  try {
    ...
    rows.forEach((row) => {
      ...
      if (usuario) {
        const compartidos = obj.Usuarios_Compartidos ? obj.Usuarios_Compartidos.split(',').map((e) => e.trim()) : [];
        if (tieneExpediente) {
          const esCompartido = compartidos.includes(usuario);
          if (!esCompartido && obj.Creado_Por !== usuario) {
            // Saltar expediente no visible para el usuario
          }
        }
      }
      ...
      if (!clientesMap.has(id)) {
        ...
        if (tieneExpediente) {
          const compartidos = obj.Usuarios_Compartidos ? obj.Usuarios_Compartidos.split(',').map((e) => e.trim()) : [];
          const esCompartido = usuario ? compartidos.includes(usuario) : true;

          if (usuario && !esCompartido && obj.Creado_Por !== usuario) {
            // Saltamos este expediente
          } else {
...
export async function agregarActuacion(
  ...
  try {
    ...
    const nextId =
      actuaciones.length > 0 ? String(Math.max(...actuaciones.map((a) => parseInt(a.ID) || 0)) + 1) : '1';
...
export async function agregarConsulta(numeroSAC, fecha, abogado, notas) {
  try {
    ...
    const nextId = consultas.length > 0 ? String(Math.max(...consultas.map((c) => parseInt(c.ID) || 0)) + 1) : '1';
...
export async function agregarFinanza(
  ...
  try {
    ...
    const nextId = finanzas.length > 0 ? String(Math.max(...finanzas.map((f) => parseInt(f.ID) || 0)) + 1) : '1';
...
export async function agregarJurisprudencia(tema, subtema, juzgado, cita) {
  try {
    ...
    const nextId = items.length > 0 ? String(Math.max(...items.map((i) => parseInt(i.ID) || 0)) + 1) : '1';
...
export async function agregarModelo(nombre, fuero, contenido) {
  try {
    ...
    const nextId = modelos.length > 0 ? String(Math.max(...modelos.map((m) => parseInt(m.ID) || 0)) + 1) : '1';
  ...
  try {
    ...
    if (filtros.usuario) {
      eventos = eventos.filter(
        (e) => e.Creado_Por === filtros.usuario || e.Compartido_Con?.includes(filtros.usuario),
      );
    }
...
export async function agregarEvento(
  ...
  try {
    ...
    const nextId = eventos.length > 0 ? String(Math.max(...eventos.map((e) => parseInt(e.ID) || 0)) + 1) : '1';
...
export async function agregarPlazo(numeroSAC, descripcion, fechaVencimiento, creadoPor) {
  try {
    ...
    const nextId = plazos.length > 0 ? String(Math.max(...plazos.map((p) => parseInt(p.ID) || 0)) + 1) : '1';
...
export async function guardarCorreccionIA(numeroSAC, tipo, promptOriginal, textoGenerado, textoCorregido, usuario) {
  try {
    ...
    const nextId = data && data.length > 1 ? String(parseInt(data[data.length - 1][0] || '0') + 1) : '1';
