...
export async function getClientes(usuario = null) {
  try {
    ...
    rows.forEach(row => {
      ...
      // CAMBIO 1: Filtrar por usuario si se proporciona
      if (usuario) {
        const compartidos = obj.Usuarios_Compartidos ? obj.Usuarios_Compartidos.split(',').map(e => e.trim().toLowerCase()) : [];
        const usuarioLower = usuario.toLowerCase();
        const esCompartido = compartidos.includes(usuarioLower);
        const esPropietario = obj.Creado_Por && obj.Creado_Por.toLowerCase() === usuarioLower;
        
        // Si no es propietario ni está compartido, saltar
        if (!esCompartido && !esPropietario) {
          return;
        }
      }
      ...
      if (!clientesMap.has(id)) {
        ...
        if (tieneExpediente) {
          const compartidos = obj.Usuarios_Compartidos ? obj.Usuarios_Compartidos.split(',').map(e => e.trim().toLowerCase()) : [];
          const usuarioLower = usuario ? usuario.toLowerCase() : null;
          const esCompartido = usuarioLower ? compartidos.includes(usuarioLower) : true;
          const esPropietario = usuario && obj.Creado_Por && obj.Creado_Por.toLowerCase() === usuarioLower;
          
          if (!usuario || esCompartido || esPropietario) {
...
export async function verificarUsuario(email, pin) {
  ...
  console.log('🔍 Email recibido (raw):', email);
  console.log('🔍 Email trimmed:', email.trim());
  console.log('🔍 Email lower:', email.trim().toLowerCase());
  ...
  try {
    ...
    // Mostrar todos los emails de la hoja
    console.log('📋 Emails en la hoja:');
    rows.forEach((row, i) => {
      const emailRow = row[emailIndex] ? row[emailIndex].toString().trim() : '';
      console.log(`  ${i + 1}: "${emailRow}"`);
    });

    const emailTrimmed = email.trim().toLowerCase();
    console.log('🔍 Buscando email exacto (lowercase):', emailTrimmed);
    
    const usuarioRow = rows.find(row => {
      const rowEmail = row[emailIndex] ? row[emailIndex].toString().trim().toLowerCase() : '';
      return rowEmail === emailTrimmed;
    });
    ...
    console.log('✅ Usuario encontrado:', usuarioRow);
    ...
    console.log('🔍 PIN guardado:', pinGuardado);
...
// CAMBIO 3: Enriquecer finanzas con nombre del cliente
export async function getFinanzasConCliente(numeroSAC = null, categoria = null, estado = null, fechaInicio = null, fechaFin = null) {
  try {
    let finanzas = await getFinanzas(numeroSAC, categoria, estado, fechaInicio, fechaFin);
    const clientes = await getClientes();
    
    // Crear mapa de SAC -> Cliente
    const sacToCliente = new Map();
    clientes.forEach(cliente => {
      if (cliente.expedientes) {
        cliente.expedientes.forEach(exp => {
          sacToCliente.set(exp.Numero_SAC, cliente.Nombre_Cliente);
        });
      }
    });
    
    // Agregar nombre del cliente a cada finanza
    finanzas = finanzas.map(f => ({
      ...f,
      Nombre_Cliente: sacToCliente.get(f.Numero_SAC) || 'Sin cliente'
    }));
    
    return finanzas;
  } catch (error) {
    console.error('❌ Error al obtener finanzas con cliente:', error.message);
    return [];
  }
}
...
function getMockData(sheetName) {
  const mockData = {
    'Clientes_y_Expedientes': [
      ['ID_Cliente', 'Nombre_Cliente', 'Telefono', 'DNI', 'Domicilio', 'Numero_SAC', 'Caratula', 'Fuero', 'ID_Carpeta_Drive', 'Usuarios_Compartidos', 'Creado_Por'],
    ...
    'Agenda': [
      ['ID', 'Numero_SAC', 'Cliente', 'Tipo', 'Titulo', 'Descripción', 'Fecha', 'Hora', 'Hora_Fin', 'Lugar', 'Recordatorio', 'Dias_Antes', 'Estado', 'Creado_Por', 'Compartido_Con', 'Notificación_Enviada', 'Google_Calendar_ID'],
    ],
    'Finanzas': [
    ...
    'Historia_Consultas': [
      ['ID', 'Numero_SAC', 'Fecha', 'Abogado_Atendio', 'Notas_Consulta'],
    ],
  };
