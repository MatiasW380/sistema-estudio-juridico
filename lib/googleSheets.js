...
export async function getClientes(usuario = null) {
  ...
  if (usuario) {
    ...
    resultado = resultado.filter((cliente) => {
      ...
      const visibleCliente = !creadoPor || creadoPor === usuarioNorm || compartidos.includes(usuarioNorm);
      const tieneExpedientesVisibles = (cliente.expedientes || []).some((exp) => {
        ...
        return !expCreado || expCreado === usuarioNorm || expCompartidos.includes(usuarioNorm) || visibleCliente;
      });
    ...
    resultado = resultado.map((cliente) => {
      ...
      const expedientes = (cliente.expedientes || []).filter((exp) => {
        const expCreado = normalizeEmail(exp.Creado_Por || '');
        const expCompartidos = parseCompartidos(exp.Usuarios_Compartidos || '');
        return visibleCliente || !expCreado || expCreado === usuarioNorm || expCompartidos.includes(usuarioNorm);
      });
...
export async function getFinanzas(numeroSAC, categoria, estado, fechaInicio, fechaFin) {
  ...
  let items = rows.slice(1).map((row) => {
    ...
    obj.Nombre_Cliente = obj.Cliente; // compatibilidad con código viejo
    return obj;
  });
...
export async function getResumenFinanzas(categoria, fechaInicio, fechaFin) {
  ...
  finanzas.forEach((f) => {
    ...
    if (est === 'Pagado') totalPagado += pagado || total;
    else if (est === 'Parcial') totalParcial += total - pagado;
    else totalPendiente += total - pagado;
  });
