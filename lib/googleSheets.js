...
export async function appendToSheet(sheetName, values) {
  try {
    ...
    // PARA AGENDA: Usar PUT con rango exacto en lugar de append
    if (sheetName === 'Agenda') {
      rows = rows.map((fila) => normalizarFilaAgenda(fila));
      
      // Leer la hoja para saber en qué fila insertar
      const readUrl = `https://sheets.googleapis.com/v4/spreadsheets/${SHEETS_ID}/values/Agenda!A:A`;
      const readResponse = await fetch(readUrl, {
        headers: { Authorization: `Bearer ${token}` },
      });
      
      if (!readResponse.ok) {
        console.error('❌ Error al leer Agenda:', readResponse.status);
        return false;
      }
      
      const readData = await readResponse.json();
      const existingRows = readData.values ? readData.values.length : 0;
      const insertRow = existingRows + 1;
      
      // Especificar el rango EXACTO donde insertar (A:Q = 17 columnas)
      const range = `Agenda!A${insertRow}:Q${insertRow + rows.length - 1}`;
      ...
      if (!response.ok) {
        console.error('❌ Error en appendToSheet (Agenda):', response.status, await response.text());
        return false;
      }
    ...
    // PARA CLIENTES_Y_EXPEDIENTES: Usar PUT con rango exacto (A:L = 12 columnas)
    if (sheetName === 'Clientes_y_Expedientes') {
      // Normalizar fila a 12 columnas
      rows = rows.map((fila) => {
        if (!Array.isArray(fila)) fila = [];
        if (fila.length > 12) fila = fila.slice(0, 12);
        while (fila.length < 12) fila.push('');
        return fila;
      });
      
      // Leer la hoja para saber en qué fila insertar
      const readUrl = `https://sheets.googleapis.com/v4/spreadsheets/${SHEETS_ID}/values/Clientes_y_Expedientes!A:A`;
      const readResponse = await fetch(readUrl, {
        headers: { Authorization: `Bearer ${token}` },
      });
      
      if (!readResponse.ok) {
        console.error('❌ Error al leer Clientes_y_Expedientes:', readResponse.status);
        return false;
      }
      ...
      // Especificar el rango EXACTO donde insertar (A:L = 12 columnas)
      const range = `Clientes_y_Expedientes!A${insertRow}:L${insertRow + rows.length - 1}`;
      const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEETS_ID}/values/${range}?valueInputOption=USER_ENTERED`;
      ...
      if (!response.ok) {
        console.error('❌ Error en appendToSheet (Clientes_y_Expedientes):', response.status, await response.text());
        return false;
      }
    ...
    // Para otras hojas: mantener el append original
    let range = sheetName;
...
export async function actualizarPerfilUsuario(email, nombre, domicilioConstituido, matricula) {
  try {
    ...
    // Buscar índices - LAS COLUMNAS YA EXISTEN
    const idxEmail = headers.indexOf('Email');
    const idxNombre = headers.indexOf('Nombre_Completo');
    const idxDomicilio = headers.indexOf('Domicilio_Constituido');
    const idxMatricula = headers.indexOf('Matricula');

    if (idxEmail === -1 || idxNombre === -1 || idxDomicilio === -1 || idxMatricula === -1) {
      console.error('Columnas faltantes en sheet Usuarios');
      return false;
    }

    // Buscar fila del usuario por email
    const emailNorm = normalizeEmail(email || '');
    if (!emailNorm) {
    ...
    for (let i = 1; i < rows.length; i += 1) {
      const rowEmail = normalizeEmail(rows[i][idxEmail] || '');
    ...
    while (rows[rowIndex].length < 7) {
      rows[rowIndex].push('');
    }

    // Guardar en las columnas existentes
    rows[rowIndex][idxNombre] = nombre || '';
    rows[rowIndex][idxDomicilio] = domicilioConstituido || '';
    rows[rowIndex][idxMatricula] = matricula || '';

    console.log('💾 Guardando:', { email, nombre, domicilioConstituido, matricula });
    
    // Escribir en Google Sheets usando la API
    const token = await getAccessToken();
    if (!token) {
    ...
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEETS_ID}/values/'Usuarios'!A1?valueInputOption=USER_ENTERED`;
    const response = await fetch(url, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
    ...
    if (!response.ok) {
      console.error('❌ Error escribiendo sheet:', response.status, await response.text());
      return false;
    }
    ...
    console.error('❌ Error en actualizarPerfilUsuario:', error.message);
