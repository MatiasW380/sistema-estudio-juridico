...
export async function getMovimientos(numeroSAC = null, fechaInicio = null, fechaFin = null, tipo = null) {
  try {
    ...
    // Filtrar por Numero_SAC
    if (numeroSAC) {
      movimientos = movimientos.filter(m => m.Numero_SAC === numeroSAC);
    }

    // Filtrar por fecha
    if (fechaInicio) {
      movimientos = movimientos.filter(m => m.Fecha_Registro >= fechaInicio);
    }
    if (fechaFin) {
      movimientos = movimientos.filter(m => m.Fecha_Registro <= fechaFin);
    }

    // Filtrar por tipo
    if (tipo) {
      movimientos = movimientos.filter(m => m.Tipo_Movimiento === tipo);
    }

    // Ordenar por fecha (más reciente primero)
    movimientos.sort((a, b) => new Date(b.Fecha_Registro) - new Date(a.Fecha_Registro));
...
export async function agregarMovimiento(numeroSAC, fechaRegistro, fechaVencimiento, tipo, detalle, montoDebe, montoHaber) {
  try {
    const movimientos = await getMovimientos();
    const nextId = movimientos.length > 0 
      ? String(Math.max(...movimientos.map(m => parseInt(m.ID_Movimiento) || 0)) + 1)
      : '1';
    ...
    const resultado = await appendToSheet('Finanzas', fila);
    return resultado;
  } catch (error) {
...
export async function getSaldo(numeroSAC) {
  try {
    const movimientos = await getMovimientos(numeroSAC);
    let totalDebe = 0;
    let totalHaber = 0;

    movimientos.forEach(m => {
      totalDebe += parseFloat(m.Monto_Debe) || 0;
      totalHaber += parseFloat(m.Monto_Haber) || 0;
    });
    ...
    console.error('❌ Error al calcular saldo:', error.message);
